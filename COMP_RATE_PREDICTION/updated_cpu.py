...
class CPUStress:
    ...
    def __init__(self, seconds: int = CORES * 5, logger: logging.Logger = None, limit: int = 100):
        ...
        while True:
            try:
                ...
                for _ in range(self.CORES):
                    busy_time = 0.01 * percentage
                    idle_time = 0.01 * (100 - percentage)
    ...
    def _measure_cpu(self,processes) -> None:
        ...
        processors = []
        while True:
            total_cpu_util = psutil.cpu_percent(interval=1)
            if total_cpu_util > self.limit:
                msg = f'CPU usage has reached {total_cpu_util}. The limit is: {self.limit}'
    ...
    def _run(self) -> None:
        ...
        try:
            ...
            for n in range(self.CORES):
                processes.append(Process(target=self._stress))
            stop_thread = False
            measure = Thread(target=self._measure_cpu,args=([processes]))
            measure.start()
            # time.sleep(1)
